...
export const ONLINE_PAYMENT_METHODS = ["MERCADO_PAGO", "PAYPAL", "ONLINE"];
export function canRetryOnlinePayment(order) {
  return (
    ONLINE_PAYMENT_METHODS.includes(order.paymentMethod) &&
    order.status !== "cancelled" &&
    ["unpaid", "failed", "processing"].includes(order.paymentStatus)
  );
}
...
export function purchaseResult(order) {
  ...
  let tone = "pending";
  const online = ONLINE_PAYMENT_METHODS.includes(order.paymentMethod);
  if (order.status === "cancelled") {
    ...
    message = online
      ? "No se confirmó el pago. Puedes intentarlo de nuevo con el mismo pedido."
    ...
    message =
      "Tu pedido fue registrado. Completa el pago en línea para confirmarlo.";
  } else {
    message =
